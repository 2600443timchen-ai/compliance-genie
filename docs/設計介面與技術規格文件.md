...
**介面位置**：透過頂部分頁切換後，佔據整個主畫面的三大區塊：敘事洞察卡片 (左側)、交叉分析矩陣 (右上) 與知識探索圖譜 (右下)。
**Gemini Data API 引用**：`POST /assistant/chat/{chatId}` (作為批次資料分析引擎)
**功能重點**：
此頁面將單一案件的聊天模式轉換為巨觀分析視角。雖然視覺呈現迥異，但底層仍然調用 Gemini 的 SSE 對話 API。系統將分析指令送出後，嘗試利用 `tryParseAiJson()` 攔截並解析 LLM 回傳的結構化 JSON 資料，接著分發至三個獨立的視覺化元件（Rich Text、HTML Table、SVG Graph）進行動態渲染。
...
| 會話列表同步 | `{BASE}/assistant/chat/list` | `GET` | 標準鑑權 Headers | `{ data: [{ _id, title }] }` | 動態獲取 Chat ID，避免硬編碼 |
| AI 對話串流 | `{BASE}/assistant/chat/{chatId}` | `POST` | `{ q, streaming: true }` | SSE `text/event-stream` | 即時串流渲染，消除等待感 |
